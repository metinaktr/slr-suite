...
slr_thematic_export <- function(te, prefix) {
  nodes <- te$Nodes; edges <- te$Edges
  a <- match(edges$from, nodes$id); b <- match(edges$to, nodes$id)
  ...
  for (side in list(left, right)) {
    ...
    for (i in side) {
      ...
      hi[i] <- cursor; lo[i] <- cursor - amount * scale
      cursor <- lo[i] - gap
    }
  }
  top_a <- hi; top_b <- hi
  ...
  for (k in seq_along(w)) {
    height <- w[k] * scale
    bands[[k]] <- c(top_a[a[k]], top_a[a[k]] - height,
  ...
  draw <- function() {
    ...
    t <- seq(0,1,length.out=100); ease <- 3*t^2-2*t^3
    x <- 0.29 + 0.42*t
    for (k in seq_along(w)) {
      z <- bands[[k]]
    ...
    for (i in c(left,right)) {
      is_left <- i %in% left; x0 <- if(is_left) 0.275 else 0.71
      graphics::rect(x0,lo[i],x0+0.015,hi[i],col=nodes$color[i],border='gray40')
      graphics::text(if(is_left) 0.267 else 0.733,(lo[i]+hi[i])/2,
                     labels=nodes$name[i],adj=if(is_left) 1 else 0,cex=0.9)
    }
    graphics::text(c(0.28,0.72),0.985,
